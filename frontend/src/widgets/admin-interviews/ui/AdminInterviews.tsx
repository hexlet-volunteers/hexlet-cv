...
export const AdminInterviews: React.FC<TProps> = (props): JSX.Element => {
  ...
  const filteredInterviews = interviews?.filter(
    (interview) =>
      ...
      interview.speaker.toLowerCase().includes(search.toLowerCase()),
  )
    ...
    <Container fluid py="md">
      ...
      <Group gap="xs" mb="md">
        ...
        <Button component={Link} href="" variant="default">
          {t('adminPage.interviews.button')}
        </Button>
      </Group>
      <Table withTableBorder verticalSpacing="md">
        <Table.Thead>
          <Table.Tr>
            <Table.Th fz="md" py="xs" w="27%">
              {t('adminPage.interviews.interviewTitle')}
            </Table.Th>
            <Table.Th fz="md" py="xs" w="27%">
              {t('adminPage.interviews.interviewSpeaker')}
            </Table.Th>
            <Table.Th fz="md" py="xs" w="31%">
              {t('adminPage.interviews.interviewVideo')}
            </Table.Th>
            <Table.Th fz="md" py="xs" w="15%" ta="center">
              {t('adminPage.interviews.interviewPublished')}
            </Table.Th>
          </Table.Tr>
        </Table.Thead>
        <Table.Tbody>
          {filteredInterviews?.map((interview) => (
            <Table.Tr key={interview.id}>
              <Table.Td>
                <Anchor underline="not-hover">{interview.title}</Anchor>
              </Table.Td>
              <Table.Td>
                <TextInput
                  value={interview.speaker}
                  readOnly
                  size="xs"
                  w={200}
